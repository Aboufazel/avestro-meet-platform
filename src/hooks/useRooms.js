...
export function useCreateRoom() {
  ...
  return useMutation({
    mutationFn: (data) => roomsService.create(data).then((r) => r.data),
    onSuccess: (room) => {
...
export function useJoinRoom(slug) {
  ...
  return useMutation({
    mutationFn: (data) => roomsService.join(slug, data).then((r) => r.data),
    onSuccess: () => {
      navigate(`/room/${slug}`)
    },
    onError: (error) => {
      const msg = error.response?.data?.detail || 'خطا در ورود به رویداد'
      toast.error(msg)
    },
  })
}
...
export function useLeaveRoom() {
  ...
  return useMutation({
    mutationFn: (slug) => roomsService.leave(slug),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['rooms'] })
    },
  })
...
export function useEndRoom() {
  ...
  return useMutation({
    mutationFn: (slug) => roomsService.end(slug),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['rooms'] })
