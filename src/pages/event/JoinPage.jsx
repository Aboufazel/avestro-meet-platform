...
export default function JoinPage() {
  ...
  if (isError || !room) return (
    <div className="min-h-screen bg-[#f4f7fb] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative bg-white border border-[#e5eaf2] rounded-[28px] p-8 max-w-md w-full text-center shadow-[0_24px_70px_rgba(20,35,65,.08)]">
        ...
        <p className="text-xl font-bold">رویداد یافت نشد</p><p className="text-sm text-[#8d98aa] mt-2 mb-7">لینک اشتباه است یا رویداد حذف شده است.</p>
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-[#101827] relative overflow-hidden py-5 sm:py-8 px-4">
      ...
      <div className="max-w-4xl mx-auto relative">
        <header className="flex items-center justify-between mb-5 px-1">
          <Link to="/" className="flex items-center gap-3"><img src="/avestro-logo.png" alt="اَوسترو" className="w-9 h-9 rounded-xl" /><div><p className="font-bold text-sm">اَوسترو میت</p><p className="text-[9px] text-[#9aa4b4] mt-0.5 tracking-wide">AVESTRO MEET</p></div></Link>
        ...
        <div className="grid lg:grid-cols-[1.05fr_.95fr] gap-4 items-stretch">
          <section className="meet-dark-panel rounded-[28px] p-7 sm:p-9 min-h-[470px] flex flex-col justify-between overflow-hidden relative">
            ...
            <div className="relative">
              ...
              <h1 className="text-3xl sm:text-4xl font-bold mt-7 leading-tight">{room.title}</h1>
            ...
            <div className="relative grid grid-cols-2 gap-3 mt-10">
              <div className="rounded-2xl bg-white/[.05] border border-white/[.07] p-4"><Users className="w-4 h-4 text-[#54d9b0]" /><p className="text-lg font-bold mt-3">{room.active_participants_count || 0}<span className="text-white/25 text-sm font-normal"> / {room.max_participants}</span></p><p className="text-[10px] text-white/35 mt-1">شرکت‌کننده</p></div>
              <div className="rounded-2xl bg-white/[.05] border border-white/[.07] p-4"><Video className="w-4 h-4 text-[#9eb6ff]" /><p className="text-sm font-medium mt-3">جلسه آنلاین</p><p className="text-[10px] text-white/35 mt-1">آماده ورود</p></div>
          ...
          <section className="bg-white border border-[#e5eaf2] rounded-[28px] p-6 sm:p-8 shadow-[0_24px_70px_rgba(20,35,65,.07)] flex flex-col justify-center">
            {room.status === 'ended' ? (
              <div className="text-center"><div className="w-14 h-14 rounded-2xl bg-[#f1f3f6] text-[#68758a] mx-auto flex items-center justify-center"><Clock className="w-6 h-6" /></div><p className="font-bold text-lg mt-5">این رویداد پایان یافته است</p><p className="text-xs text-[#8d98aa] mt-2 mb-6">برای مشاهده جلسات دیگر به صفحه اصلی برگردید.</p><Link to="/"><Button variant="secondary" className="!text-[#101827] !border-[#e5eaf2] !bg-white hover:!bg-[#edf2ff]">بازگشت به خانه</Button></Link></div>
            ) : mode === 'select' ? (
              <div>
                ...
                <h2 className="text-xl font-bold">ورود به جلسه</h2><p className="text-xs text-[#8d98aa] mt-2 mb-6">روش ورود خود را انتخاب کنید.</p>
                {token ? (
                  <>
              ...
              <div>
                ...
                <h2 className="text-xl font-bold">ورود به عنوان مهمان</h2><p className="text-xs text-[#8d98aa] mt-2 mb-6">نامی که سایر شرکت‌کنندگان در جلسه می‌بینند.</p>
