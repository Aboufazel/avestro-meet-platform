...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    api.addListener('incomingMessage', ({ from, message, nick }) => {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString(), sender: nick || from, text: message, time: now }])
    })
  ...
  function toggleMute() { apiRef.current?.executeCommand('toggleAudio') }
  function toggleVideo() { apiRef.current?.executeCommand('toggleVideo') }
  function toggleShareScreen() { apiRef.current?.executeCommand('toggleShareScreen') }
  function raiseHand() { apiRef.current?.executeCommand('toggleRaiseHand'); toast('دست بلند شد ✋') }
  function handleLeave() { apiRef.current?.executeCommand('hangup'); navigate('/') }
  ...
  return (
    <div className="h-screen bg-olive-950 flex flex-col overflow-hidden">
      ...
      {/* Header */}
      <div className="h-14 bg-olive-900 border-b border-olive-800 flex items-center justify-between px-4 shrink-0">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="flex-1 bg-black relative overflow-hidden">
          ...
          />
          {!isConnected && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-olive-950 z-10">
              ...
              {activePanel === 'participants' && (
                <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
                  {participants.length === 0 ? (
                    ...
                    participants.map((p) => (
                      <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-olive-800 transition-colors">
              ...
              {activePanel === 'chat' && (
                <>
                  <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
                    {messages.map((msg) => (
                      <div key={msg.id} className={`flex flex-col gap-1 ${msg.isMine ? 'items-start' : 'items-end'}`}>
                        <div className="flex items-center gap-2">
                          <span className="text-olive-500 text-xs font-medium">{msg.sender}</span>
      ...
      {/* Controls */}
      <div className="h-16 bg-olive-900 border-t border-olive-800 flex items-center justify-between px-4 sm:px-6 shrink-0">

        {/* Right Controls */}
        <div className="flex gap-1.5 sm:gap-2">
          <ControlBtn active={!isMuted} onClick={toggleMute} icon={isMuted ? MicOff : Mic} tooltip={isMuted ? 'روشن کردن میکروفون' : 'خاموش کردن میکروفون'} />
          <ControlBtn active={!isVideoOff} onClick={toggleVideo} icon={isVideoOff ? VideoOff : Video} tooltip={isVideoOff ? 'روشن کردن دوربین' : 'خاموش کردن دوربین'} />
          <ControlBtn active={false} onClick={toggleShareScreen} icon={Monitor} tooltip="اشتراک‌گذاری صفحه" className="hidden sm:flex" />
          <ControlBtn active={false} onClick={raiseHand} icon={Hand} tooltip="دست بلند کردن" className="hidden sm:flex" />
        </div>
        ...
        {/* Left Controls */}
        <div className="flex gap-1.5 sm:gap-2">
          <ControlBtn active={activePanel === 'participants'} onClick={() => togglePanel('participants')} icon={Users} tooltip="شرکت‌کنندگان" />
          <ControlBtn active={activePanel === 'chat'} onClick={() => togglePanel('chat')} icon={MessageSquare} tooltip="گفتگو" />
        </div>
